...
def part1(puzzle_in: str):
	...
	for line in puzzle_in.splitlines():
		...
		if   action == 'turn on': grid[x0:x1+1, y0:y1+1] = True
		elif action == 'turn off': grid[x0:x1+1, y0:y1+1] = False
		elif action == 'toggle': grid[x0:x1+1, y0:y1+1] = ~grid[x0:x1+1, y0:y1+1]
...
def part2(puzzle_in: str):
	...
	for line in puzzle_in.splitlines():
		...
		if   action == 'turn on': grid2[x0:x1+1, y0:y1+1] += 1
		elif action == 'turn off':
			grid2[x0:x1+1, y0:y1+1] -= 1
			grid2[grid2 < 0] = 0
		elif action == 'toggle': grid2[x0:x1+1, y0:y1+1] += 2
